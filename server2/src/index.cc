...
{
	IndexLine::IndexLine(const uint32_t max_line_length):
	...
	bool IndexLine::merge(const vector<uint32_t>& new_docs){
		read_existing();
	...
	bool IndexLine::remove(const vector<uint32_t>& new_docs){
		read_existing();
		// cout << "Start In Length: " << in_length << " Out Length: " << out_length <<" Existing size: " << existing_docs_.size() << " New size: " << new_docs.size() << " Merged size: " << merged_docs_.size() <<endl;
	...
	Index::Index(const Registry& registry):
	...
	void Index::merge(kc::TinyHashMap& hashes,bool remove){
		if (remove){
	...
	bool Index::batch(deque<Command*>& commands){
		// Posting* postings = new Posting(registry_);
		// postings->batch(commands);
		// delete postings;
