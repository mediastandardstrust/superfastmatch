...
TEST_F(CommandTest,QueryTest){
  ...
  CommandPtr addDoc2 = registry_.getQueueManager()->createCommand(AddDocument,2,1,"","","text=This+is+a+test");
  CommandPtr addDoc3 = registry_.getQueueManager()->createCommand(AddDocument,2,2,"","","text=This+is+a+test");
  EXPECT_EQ(3U,registry_.getQueueManager()->processQueue());
  CommandPtr associateDoc1 = registry_.getQueueManager()->createCommand(AddAssociation,0,0,"1","2","");
  EXPECT_EQ(1U,associateDoc1->getDocumentQuery()->getSourceDocPairs().size());
  EXPECT_EQ(2U,associateDoc1->getDocumentQuery()->getTargetDocPairs().size());
}
